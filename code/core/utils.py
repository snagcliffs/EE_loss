...
def get_AUC_grid(a,b,nq=50,q_min=1e-3,q_max=0.1,log_spacing=True,Q=None):

    if Q is None: 
        if log_spacing: Q = np.logspace(np.log10(q_min), np.log10(q_max), nq)
        else: Q = np.linspace(q_min,q_max,nq+1)[1:]
...
@nb.njit
def F1_vals(a,b,q=0.1,nb=101,w=1.0):
    """
    ...
    b_hat_q = np.percentile(b, 100*(1-q))
    b_min = np.min(b)
    b_max = np.max(b)
    B_hat = np.linspace(b_hat_q - w*(b_hat_q-b_min),
                        b_hat_q + w*(b_max-b_hat_q),nb)

    # F1 scores for each b_hat in B_hat
    F1_scores = np.array([F1_score(a,b,a_hat,b_hat) for b_hat in B_hat])
